...
export default function AdminCertificatesPage() {
  ...
  return (
    <div className={styles._1}>
      ...
      {isIssuer && selectedIds.size > 0 && (
        <div className="mb-4 flex items-center gap-3 rounded-2xl border border-amber-200 bg-amber-50/50 px-4 py-3 dark:border-amber-900 dark:bg-amber-950/10">
          ...
          <button
            ...
            className="rounded-xl bg-primary px-4 py-2 text-xs font-bold text-white hover:bg-primary-hover disabled:opacity-50 transition-all cursor-pointer shadow-2xs active:scale-95"
          >
            {batchApproving ? t("adminCertificates.batch.signing") : `${t("adminCertificates.batch.signAll")} (${selectedIds.size})`}
          </button>
          <button
            onClick={() => setSelectedIds(new Set())}
            className="rounded-xl border px-3 py-2 text-xs font-bold text-gray-500 dark:text-gray-400 cursor-pointer"
          >
      ...
      {/* Confirmation Modal for Batch Signing */}
