    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">Các lần import sinh viên từ file CSV.</p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-gray-400 text-xs">Chưa có lần import nào.</div>
      ) : (
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-gray-50 dark:bg-gray-800/50 border-b border-gray-200/60 dark:border-gray-800/60">
                <th className="text-left px-4 py-3 font-bold text-gray-600">File</th>
                <th className="text-left px-4 py-3 font-bold text-gray-600">Người tạo</th>
                <th className="text-center px-4 py-3 font-bold text-gray-600">Tổng</th>
                <th className="text-center px-4 py-3 font-bold text-gray-600">Thành công</th>
                <th className="text-center px-4 py-3 font-bold text-gray-600">Thất bại</th>
                <th className="text-left px-4 py-3 font-bold text-gray-600">Thời gian</th>
              </tr>
            </thead>
            <tbody>
              {batches.map((b) => (
                <tr key={b.id} className="border-b border-gray-100 dark:border-gray-800/40 hover:bg-gray-50 dark:hover:bg-gray-800/30">
                  ...
                  <td className="px-4 py-3 text-gray-500">{b.created_by_name}</td>
                  <td className="px-4 py-3 text-center text-gray-900 dark:text-white font-bold">{b.total_rows}</td>
                  <td className="px-4 py-3 text-center">
                    <span className="text-green-600 font-bold">{b.success_rows}</span>
                  </td>
                  <td className="px-4 py-3 text-center">
                    <span className={`font-bold ${b.failed_rows > 0 ? 'text-red-600' : 'text-gray-400'}`}>
                      {b.failed_rows}
                    </span>
                  </td>
