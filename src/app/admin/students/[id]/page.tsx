...
export default function StudentDetailPage() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(t("admin.students.update_success"));
    } catch (err) {
      setError(err instanceof Error ? err.message : t("admin.students.update_failed"));
    } finally {
      setSaving(false);
    }
  };

  if (loading) return <div className="p-8 text-xs text-gray-400 dark:text-gray-500">{t("common.loading_student")}</div>;

  return (
    <div className="mx-auto max-w-xl space-y-6 p-6">
      <div>
        <h1 className="text-xl font-black uppercase tracking-tight text-gray-900 dark:text-white">{t("admin.students.detail")}</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 rounded-3xl border border-gray-200/60 bg-white p-6 dark:border-gray-800/60 dark:bg-gray-900">
        <label className="block text-xs font-bold">{t("admin.students.full_name")}
        ...
        <label className="block text-xs font-bold">{t("admin.students.email")}
        ...
        <div className="flex gap-3">
          ...
          <button type="button" onClick={() => router.push("/admin/students")} className="rounded-xl bg-gray-100 px-4 py-2.5 text-xs font-bold text-gray-500 dark:bg-gray-800 dark:text-gray-400">{t("common.back")}</button>
        </div>
      </form>
    </div>
  );
