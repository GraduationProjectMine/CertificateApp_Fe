...
export default function CreateStudentPage() {
  ...
  return (
    <div className="max-w-lg mx-auto p-6 space-y-6">
      <div>
        <h1 className="text-xl font-black text-gray-900 dark:text-white uppercase tracking-tight">{t("adminStudentCreate.title")}</h1>
        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{t("adminStudentCreate.description")}</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-900 border border-gray-200/60 dark:border-gray-800/60 rounded-3xl p-6 space-y-4">
        <div>
          <label className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1.5">{t("adminStudentCreate.fullNameLabel")} *</label>
          <input
            type="text"
            ...
            placeholder={t("adminStudentCreate.fullNamePlaceholder")}
          />
        </div>
        <div>
          <label className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1.5">{t("adminStudentCreate.emailLabel")} *</label>
          <input
            type="email"
            className="w-full px-3 py-2.5 rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 text-xs focus:outline-none focus:ring-1 focus:ring-primary"
            value={form.email}
            onChange={(e) => setForm({ ...form, email: e.target.value })}
            placeholder={t("adminStudentCreate.emailPlaceholder")}
          />
        </div>
        <div>
          <label className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1.5">{t("adminStudentCreate.passwordLabel")} *</label>
          <div className="relative">
            <input
              ...
              placeholder={t("adminStudentCreate.passwordPlaceholder")}
            />
            <button
              type="button"
              ...
              title={showPassword ? t("adminStudentCreate.hidePassword") : t("adminStudentCreate.showPassword")}
            >
              {showPassword ? (
        ...
        <div className="flex gap-3">
          ...
          >
            {submitting ? t("adminStudentCreate.creating") : t("adminStudentCreate.submitLabel")}
          </button>
          <button
            type="button"
