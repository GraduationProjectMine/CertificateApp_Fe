...
export default function StudentProfile() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setError("");
    setSuccess("");
    if (!name.trim()) { setError(t("student.profile.enter_name")); return; }
    setSaving(true);
    try {
      const res = await studentApi.updateProfile({ name: name.trim(), email: email.trim() || undefined });
      setSuccess(t("student.profile.update_success"));
      setEditing(false);
    } catch (err) {
      setError(err instanceof Error ? err.message : t("student.profile.update_failed"));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className={styles._1}>
      <div className="flex items-center justify-between mb-6">
        <h1 className={styles._2}>{t("student.profile.title")}</h1>
        {!editing && (
      ...
      {editing ? (
        <form onSubmit={handleSave} className="rounded-2xl border border-gray-200/60 bg-white p-5 text-sm dark:border-gray-800/60 dark:bg-gray-900 space-y-4">
          <div>
            <label className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1.5">{t("student.profile.full_name")}</label>
          ...
          <div>
            <label className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1.5">{t("student.profile.email")}</label>
        ...
        <div className="rounded-2xl border border-gray-200/60 bg-white p-5 text-sm dark:border-gray-800/60 dark:bg-gray-900">
          <dl className="space-y-3">
            <div>
              <dt className="text-xs font-bold uppercase text-gray-400 dark:text-gray-500">{t("student.profile.full_name")}</dt>
              <dd className={styles._3}>{user?.name || t("common.no_info")}</dd>
            </div>
            <div>
              <dt className="text-xs font-bold uppercase text-gray-400 dark:text-gray-500">{t("student.profile.email")}</dt>
              <dd className={styles._3}>{user?.email || t("common.no_info")}</dd>
            </div>
            <div>
              <dt className="text-xs font-bold uppercase text-gray-400 dark:text-gray-500">{t("student.profile.account_id")}</dt>
              <dd className={styles._3}>{user?.id || t("common.no_info")}</dd>
            </div>
          </dl>
        </div>
      )}
      ...
      <div className="mt-6">
        <button onClick={() => router.push("/student/profile/change-password")}
          ...
          {t("student.profile.change_password")}
        </button>
      </div>
    </div>
  );
